...
export class RelayController extends PeerPool {
  ...
  private addSender(
    address: string,
    dummyPeer: SignalingPeer,
    dummyProtocol: DummyInProtocol,
    soupPeer: IncomingSoupPeer,
  ) {
  ...
  override removeListener(client: ISignalingPeer, address: string): void {
    ...
    if (senderInfo) {
      if (senderInfo.soupPeer.state == SoupPeerConnectionState.Connected) {
        ...
        this.mLog.log(
          `Address ${address} was freed but an active sender remains.`,
        );
      } else {
        ...
        this.mLog.warn(
          `Removing sender ${address} because the address was freed before mediasoup connected`,
        );
  ...
  private removeSender(address: string, sender: Sender) {
    ...
    if (!this.mSenders[address]) {
      this.mLog.warn(
        "Tried to cleanup sender  " +
          sender.protocol.getIdentity() +
          " address " +
          address +
          " but no sender found.",
      );
  ...
  private removeReceiver(address: string, receiver: Receiver) {
    ...
    if (!this.mReceivers[address]) {
      this.mLog.warn(
        "Tried to cleanup receiver  " +
          receiver.protocol.getIdentity() +
          " address " +
          address +
          " but no receiver found.",
      );
      return;
    }
    this.mLog.log(`Receiver for ${address} removed. `);

    this.mReceivers[address] = this.mReceivers[address].filter(
      (item) => item !== receiver,
    );
    if (this.mReceivers[address].length === 0 && !this.mSenders[address]) {
      this.mLog.log(
        "No receivers left for " + address + ". Cleaning up receiver list",
      );
      delete this.mReceivers[address];
    }
  }
  ...
  private addReceiver(
    address: string,
    dummyPeer: SignalingPeer,
    dummyProtocol: DummyOutProtocol,
    soupPeer: OutgoingSoupPeer,
  ) {
  ...
  public async createNewIncomingRelay(
    address: string,
    incomingSignalingPeer: ISignalingPeer,
  ) {
    ...
    const dummyProtocol = new DummyInProtocol(
      soupPeer,
      this.mSoupServer,
      loggerBase,
    );
  ...
  public hasSender(receiverOrSenderAddress: string): boolean {
    const senderAddress = RelayController.toSenderAddress(
  ...
  public static toSenderAddress(receiverOrSenderAddress: string) {
    let senderAddress = receiverOrSenderAddress.substring(
      0,
      receiverOrSenderAddress.length - 4,
    );
    senderAddress = senderAddress + "_snd";
    return senderAddress;
  }

  public async createNewOutgoingRelay(
    address: string,
    clientPeer: ISignalingPeer,
  ) {
    //create incoming peer
    ...
    const soupPeer = await this.mSoupServer.createOutgoingPeer(
      sender.soupPeer,
      loggerBase,
    );

    const dummyProtocol = new DummyOutProtocol(
      soupPeer,
      this.mSoupServer,
      loggerBase,
    );
  ...
  public onConnectionRequest(
    client: ISignalingPeer,
    address: string,
  ...
  public logStatus() {
    this.mLog.logv(this.count() + " connections left in pool ");
  }
}
