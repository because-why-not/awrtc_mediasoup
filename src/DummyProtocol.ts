...
abstract class DummyProtocol extends Protocol {
  ...
  private evtToString(evt: NetworkEvent): string {
    ...
    if (evt.MessageData != null) {
      const chars = new Uint16Array(
        evt.MessageData.buffer,
        evt.MessageData.byteOffset,
        evt.MessageData.byteLength / 2,
      );
...
export class DummyInProtocol extends DummyProtocol {
    ...
    try {
      ...
      if (typeof json === "number") {
        ...
        if (json.type === "offer") {
          ...
          this.mLog.error(
            "Received an answer from an incoming connection. This should not be possible. msg: " +
              msg,
          );
        }
      } else if (
  ...
  public triggerConnectionRequest(address: string): void {
    ...
    this.mListener.onNetworkEvent(
      new NetworkEvent(NetEventType.NewConnection, id, address),
    );
  }
}
