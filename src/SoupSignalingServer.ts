...
export class SoupSignalingServer {
  mLog: SLogger;
  mSignalingServer: WebsocketNetworkServer;

  constructor() {}

  private async initConfig(config: RelayServerConfig): Promise<void> {
    if (config.announcedAddressFromDomain) {
      ...
      try {
        ...
        console.error(
          `Failed to resolve domain '${config.announcedAddressFromDomain}':`,
          err,
        );
        return;
      }
      ...
      console.log(
        `Resolved announcedAddressFromDomain '${config.announcedAddressFromDomain}' to IP: ${resolvedIp}`,
      );
  ...
  public async init(config: RelayServerConfig) {
    this.mLog = new SLogger("sig");
    if (
      typeof config.log_verbose === "undefined" ||
      config.log_verbose == true
    ) {
      this.mLog.log(
        'Using verbose log. This might lower performance. Add "log_verbose": false to config.json to deactivate this.',
      );
      this.mLog.setLogLevel(true);
    } else {
      this.mLog.setLogLevel(false);
    }
    await this.initConfig(config);
    this.mSignalingServer = new WebsocketNetworkServer(this.mLog);
    ...
    config.apps.forEach((app) => {
      if (app.relay) {
        this.mSignalingServer.addPeerPool(
          app.path,
          new RelayController(app, soupServer, this.mLog.createSub(app.path)),
        );
      } else {
        this.mSignalingServer.addPeerPool(
          app.path,
          new DefaultPeerPool(app, this.mLog.createSub(app.path)),
        );
      }
    });
    ...
    if (tokenManager.isActive()) {
      this.mLog.log(
        "Admin token set in config.json. Connections will be blocked by default unless a valid user token is used.",
      );
    } else {
      this.mLog.log("No admin token set. The server allows all connections.");
    }
    ...
    const defaultRequest = (
      ...
      this.mLog.log(
        `Request received from IP: ${req.socket.remoteAddress}:${req.socket.remotePort} to url ${req.url}`,
      );
    ...
    this.mSignalingServer.addSocketServer(webSocketServer, () => true);
    ...
    if (config.httpsConfig) {
      ...
      httpsServer.listen(options, function () {
        console.log(
          "secure websockets/https listening on ",
          httpsServer.address(),
        );
      });
      ...
      //incoming websocket connections will be handled by signalingServer
      this.mSignalingServer.addSocketServer(webSocketSecure, () => true);
    }
  }
}
